...
public:
  ...
  bool isGE11MuonLoose( const Muon* muonRef );
  bool isGE11MuonTight( const Muon* muonRef );
  bool isGE21MuonLoose( const Muon* muonRef );
  bool isGE21MuonTight( const Muon* muonRef );
  ...
  bool isGE11MuonSelNew(const GEMGeometry* gemGeo_, const reco::Muon *mu, double dEtaCut, double dPhiCut, double dPhiBendCut);
...
private:
  ...
  bool b_muon_isGE11MuonLoose, b_muon_isGE11MuonTight, b_muon_isGE21MuonLoose, b_muon_isGE21MuonTight, b_muon_isGE11MuonSelNew;
...
void MuonAnalyser::fillBranches(TTree *tree, TLorentzVector tlv, edm::RefToBase<reco::Muon> muref, bool isSignal, int pdgId)
{
  ...
  b_muon_isGE11MuonLoose = 0; b_muon_isGE11MuonTight = 0; b_muon_isGE21MuonLoose = 0; b_muon_isGE21MuonTight = 0; b_muon_isGE11MuonSelNew = 0;
  ...
  if (mu){
    ...
    b_muon_isGE11MuonSelNew = isGE11MuonSelNew(gemGeo_, mu, dEtaCut_, dPhiCut_, dPhiBendCut_);
    
    b_muon_isGEMMuon = mu->isGEMMuon();
    b_muon_isGE11MuonLoose = isGE11MuonLoose(mu);
    b_muon_isGE11MuonTight = isGE11MuonTight(mu);
    b_muon_isGE21MuonLoose = isGE21MuonLoose(mu);
    b_muon_isGE21MuonTight = isGE21MuonTight(mu);
...
//------------------------
/*
bool MuonAnalyser::isGE11MuonSelNew(const GEMGeometry* gemGeo_, const reco::Muon *muon, double dEtaCut, double dPhiCut, double dPhiBendCut)
{
  bool result = false;
  bool isGEM = muon->isGEMMuon();
  ...
  if(isGEM){
    ...
    const std::vector<reco::MuonChamberMatch>& chambers = muon->matches();
    for( std::vector<reco::MuonChamberMatch>::const_iterator chamber = chambers.begin(); chamber != chambers.end(); ++chamber ){
     for (auto segment : chambers.gemMatches){   
        if (chamber->detector() == 4){
          auto gemSegment = (*segment.gemSegmentRef);
          if (gemSegment.gemDetId().station() == 1){
          for ( std::vector<reco::MuonSegmentMatch>::const_iterator segment = chamber->gemMatches.begin(); segment != chamber->gemMatches.end(); ++segment ){
              ...
              const GEMChamber * GE11chamber = gemGeo_->chamber(chamber->id);
              
              GlobalPoint trk_glb_coord = GE11chamber->toGlobal(trk_loc_coord);
              GlobalPoint seg_glb_coord = GE11chamber->toGlobal(seg_loc_coord);
              
              double segDPhi = segment->me0SegmentRef->deltaPhi();
              double trackDPhi = GE11chamber->computeDeltaPhi(trk_loc_coord, trk_loc_vec);
...
bool MuonAnalyser::isGE11MuonLoose( const reco::Muon *mu ) {
    bool result = false;
    float ge11SegX = 100;
  for (auto chamber : mu->matches()){
    for (auto segment : chamber.gemMatches){
      if (chamber.detector() == 4){
        ...
        if (gemSegment.gemDetId().station() == 1){
          ge11SegX = abs( chamber.x - segment.x );
        if (ge11SegX < abs(b_muon_GE11deltaX)){
          float GE11deltaX    = ( chamber.x - segment.x );
          float GE11deltaY    = ( chamber.y - segment.y );
          float GE11pullX  = (chamber.x - segment.x) / std::sqrt(chamber.xErr + segment.xErr);
          float GE11pullY  = (chamber.y - segment.y) / std::sqrt(chamber.yErr + segment.yErr);
          float GE11dPhi = atan(chamber.dXdZ) - atan(segment.dXdZ);
          if (GE11deltaX < 3 || GE11pullX < 3)
            if (GE11deltaY < 3 || GE11pullY < 3)
              if (GE11dPhi < 0.5)
                   result = true;
	    }
      }
...
bool MuonAnalyser::isGE11MuonTight( const reco::Muon *mu ) {
    bool result = false;
    float ge11SegX = 100;
  for (auto chamber : mu->matches()){
    for (auto segment : chamber.gemMatches){
      if (chamber.detector() == 4){
        ...
        if (ge11SegX < abs(b_muon_GE11deltaX)){
          ...
          float GE11pullY  = (chamber.y - segment.y) / std::sqrt(chamber.yErr + segment.yErr);
          float GE11dPhi = atan(chamber.dXdZ) - atan(segment.dXdZ);
          if (GE11deltaX < 2 || GE11pullX < 2)
            if (GE11deltaY < 2 || GE11pullY < 2)
              if (GE11dPhi < 0.3)
                   result = true;
	    }
      }
...
bool MuonAnalyser::isGE21MuonLoose( const reco::Muon *mu ) {
    bool result = false;
    float ge21SegX = 100;
  for (auto chamber : mu->matches()){
    for (auto segment : chamber.gemMatches){
      if (chamber.detector() == 4){
        ...
        if (gemSegment.gemDetId().station() == 2){
          ge21SegX = abs( chamber.x - segment.x );
        if (ge21SegX < abs(b_muon_GE21deltaX)){
          float GE21deltaX    = ( chamber.x - segment.x );
          float GE21deltaY    = ( chamber.y - segment.y );
          float GE21pullX  = (chamber.x - segment.x) / std::sqrt(chamber.xErr + segment.xErr);
          float GE21pullY  = (chamber.y - segment.y) / std::sqrt(chamber.yErr + segment.yErr);
          float GE21dPhi = atan(chamber.dXdZ) - atan(segment.dXdZ);
          if (GE21deltaX < 3 || GE21pullX < 3)
            if (GE21deltaY < 3 || GE21pullY < 3)
              if (GE21dPhi < 0.5)
                   result = true;
	    }
      }
...
bool MuonAnalyser::isGE21MuonTight( const reco::Muon *mu ) {
    bool result = false;
    float ge21SegX = 100;
  for (auto chamber : mu->matches()){
    for (auto segment : chamber.gemMatches){
      if (chamber.detector() == 4){
        ...
        if (ge21SegX < abs(b_muon_GE21deltaX)){
          ...
          float GE21pullY  = (chamber.y - segment.y) / std::sqrt(chamber.yErr + segment.yErr);
          float GE21dPhi = atan(chamber.dXdZ) - atan(segment.dXdZ);
          if (GE21deltaX < 2 || GE21pullX < 2)
            if (GE21deltaY < 2 || GE21pullY < 2)
              if (GE21dPhi < 0.3)
                   result = true;
	    }
      }
...
void MuonAnalyser::setBranches(TTree *tree)
{
  ...
  tree->Branch("muon_isGE11MuonLoose", &b_muon_isGE11MuonLoose, "muon_isGE11MuonLoose/O");
  tree->Branch("muon_isGE11MuonTight", &b_muon_isGE11MuonTight, "muon_isGE11MuonTight/O");
  tree->Branch("muon_isGE21MuonLoose", &b_muon_isGE21MuonLoose, "muon_isGE21MuonLoose/O");
  tree->Branch("muon_isGE21MuonTight", &b_muon_isGE21MuonTight, "muon_isGE21MuonTight/O");
  tree->Branch("muon_isGE11MuonSelNew", &b_muon_isGE11MuonSelNew, "muon_isGE11MuonSelNew/O");
